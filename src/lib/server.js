...
async function createClientForSession(session) {
  ...
  client.on('message_create', async (message) => {
    try {
      ...
      console.log(`📩 [${chat.isGroup ? 'GROUP' : 'DM'}] ${message.fromMe ? 'ME' : 'THEM'}: "${text.substring(0, 40)}"`);
      
      // Only respond to YOUR OWN messages
      if (!message.fromMe) return;
      
      // In-group command: @everyone or /tagall
      if (chat.isGroup && (text.startsWith('@everyone') || text.startsWith('/tagall'))) {
        console.log(`🎯 Tag command detected!`);
        try {
          ...
          if (!me || !me.isAdmin) {
            console.log(`❌ Not an admin in this group`);
            await message.reply('❌ You must be an admin to tag everyone.');
            return;
          }
          
          console.log(`✅ Admin confirmed, proceeding to tag...`);
          
          // Extract the message after the command
          let messageText = text.replace(/^(@everyone|\/tagall)\s*/, '').trim();
          if (!messageText) {
          ...
          console.log(`✅ Tagged ${mentions.length} members in ${chunks.length} message(s)`);
          return;
        } catch (err) {
          console.error('Error tagging everyone:', err);
          await message.reply(`❌ Failed to tag everyone: ${err.message}`);
          return;
        }
      }
      ...
      if (text === '/help') {
        const helpText = `🤖 *WhatsApp Bot Commands*\n\n` +
          `*In Groups:*\n` +
          `👥 @everyone [message] - Tag everyone in the group\n` +
          ...
          `*Examples:*\n` +
          `In group: @everyone Meeting at 3pm!\n` +
          `Personal: /broadcast Family | Happy New Year! 🎉`;
...
app.get('/api/session/:id/groups', bearerAuth, async (req, res) => {
  ...
  // Return empty - use in-group commands instead
  res.json({ 
    groups: [],
    message: 'Use @everyone or /tagall directly in your WhatsApp groups'
  });
});
...
app.post('/api/session/:id/everyone', bearerAuth, async (req, res) => {
  ...
  const session = sessions.get(req.params.id);
  console.log(`📢 Broadcast request for session: ${req.params.id}, Group: ${groupId}`);
  
  if (!session || session.state !== 'open') {
    console.log(`❌ Session not ready for broadcast. State: ${session?.state}`);
    return res.status(400).json({ error: 'Session not ready' });
  }
  ...
  try {
    ...
    console.log(`Fetching participants for: ${chat.name}`);
    const participants = await chat.participants;
    ...
    const me = participants.find(p => p.id.user === myNumber);
    if (!me || !me.isAdmin) {
      console.log(`❌ Not an admin in group: ${chat.name}`);
    ...
    console.log(`Sending to ${mentions.length} members...`);
    ...
    console.log(`✅ Broadcast complete: ${chunks.length} messages sent`);
